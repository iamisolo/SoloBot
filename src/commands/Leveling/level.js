    ...
    try {
      ...
      if (sub === 'toggle') {
        ...
        return InteractionHelper.safeEditReply(interaction, {
          embeds: [
            createEmbed({
              title: '⚙️ Leveling Toggled',
              description: `System is now **${enabled ? 'ENABLED' : 'DISABLED'}**`,
              color: enabled ? 'success' : 'error'
            })
          ]
        });
          ...
          embeds: [
            new EmbedBuilder()
              ...
              .addFields(
                ...
                {
                  name: 'Channel',
                  value: data?.levelUpChannel ? `<#${data.levelUpChannel}>` : 'Not set',
                  inline: true
                },
                {
                  name: 'XP Range',
      ...
      if (sub === 'setup') {
        ...
        if (!botHasPermission(channel, ['SendMessages', 'EmbedLinks'])) {
          return InteractionHelper.safeEditReply(interaction, {
            embeds: [errorEmbed('Permission Error', 'Bot needs SendMessages + EmbedLinks')]
          });
        }
      ...
      logger.info(`[LEVEL] Used by ${interaction.user.tag} | ${Date.now() - start}ms`);
